...
export default class Servers extends Command {
    ...
    public static summary = `Execute command in the ${chalk.cyan('servers')} image. Empty argument starts a shell session`;
    ...
    public async run (): Promise<void> {
        ...
        this.log(chalk.yellow(
            "You're working inside the 'servers' application container of this project.",
        ));
        ...
        if (this.argv.length) {
            execSync(
                `
                    docker run -it --rm \
                        --entrypoint "" \
                        --name mission-control-servers \
                        -v "${rootDir}:/app" \
                        -w /app \
                        buzzingpixel/mission-control-servers bash -c "${this.argv.join(' ')}";
                `,
                { stdio: 'inherit' },
            );
        ...
        execSync(
            `
                docker run -it --rm \
                    ...
                    -v "${rootDir}:/app" \
                    -w /app \
                    buzzingpixel/mission-control-servers bash;
            `,
            { stdio: 'inherit' },
        );
    }
