...
import { KeyIcon, ChevronDownIcon } from '@heroicons/react/20/solid';
import { SshKeysWithViewOptions } from './SshKeys';
import { useArchiveSelectedSshKeysMutation } from './SshKeyData';
import SshKeyListItem from './SshKeyListItem';
...
const SshKeyList = (
    ...
    const archiveSelectedMutation = useArchiveSelectedSshKeysMutation(
    ...
    if (items.length < 1) {
        return (
            <NoResultsAddItem
                icon={<KeyIcon />}
                headline="No Keys match your filters"
            />
        );
    }
    ...
    const withSelectedOpacity = withSelectedDisabled
        ? 'opacity-40'
        : 'opacity-100';
